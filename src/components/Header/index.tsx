  ...
  return (
    ...
    >
      <section className="max-w-screen-desktop self-center w-full">
        <div className="flex gap-2 px-3 pt-3 pb-1 w-full justify-between items-center">
          {backTo && (
            <Link
              href={backTo}
              className="flex items-center w-[22px] justify-center flex-shrink-0"
            >
              <RxCaretLeft size={45} className="flex-shrink-0 text-white" />
            </Link>
          )}
          {hideSearch ? (
            label ? (
              <div className="max-w-[300px] flex overflow-scroll scrollbar-hide">
                <p className="text-3xl font-light leading-1 whitespace-nowrap text-white h-full items-center justify-center ">
                  {label}
                </p>
              </div>
            ) : null
          ...
          >
            ...
            <data className="absolute z-30 -top-1 -right-1 bg-yellow text-white font-semibold rounded-full w-5 h-5 flex items-center justify-center text-xs">
              {totalCartQuantity}
            </data>
          </button>
        </div>
        {/* {showAddress &&
          (!currentUser?.id ? (
            ...
            >
              <MapPin weight="fill" size={18} className="flex-shrink-0 mr-2" />
